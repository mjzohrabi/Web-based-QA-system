<div class="main-content-left">
    <div class="main-content-wrapper<?=_css?>">
        <span style="font-size: 15pt;"><?=_tagged_questions?></span>
    </div>
    <?php
    $results=PageModel::get_tagged_questions($records['tag'],$records['start'],$records['count']);
    ...
    <div class="pagination<?=_css?>">
        ...
        if($records['index']>5){
            ...
            <?php }
                ...
                <a href="/<?=_main_lang?>/questions/tag/<?=tag_link($records['tag'])?>" class="pagination-btn">1</a>
            <?php }
            ...
            for($i=$records['index']-3;$i<=$records['index']+3;$i++){
                ...
                <?php }
                    ...
                    <a href="/<?=_main_lang?>/questions/tag/<?=tag_link($records['tag'])?>/<?=$i?>" class="pagination-btn"><?=$i?></a>
                <?php }
                if($i==$records['ceil']){
                    break;
                }
            ...
            if($records['index']+3<$records['ceil']){
                ...
                ?>
                <a href="/<?=_main_lang?>/questions/tag/<?=tag_link($records['tag'])?>/<?=$records['ceil']?>" class="pagination-btn"><?=$records['ceil']?></a>
            <?php }
        }
        else {
            if($records['ceil']>8){
...
<script>
    $(function(){
        $('title').html('Questions tagged with '+'<?=$records['tag']?>');
    })
